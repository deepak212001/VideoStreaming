...
const getVideoComments = asyncHandler(async (req, res) => {
  ...
  const limitNum = Math.min(50, Math.max(1, Number(req.query.limit) || 30));
  ...
  for (const com of comments) {
    ...
    com.isLiked = userId
      ? !!(await Like.findOne({comment: com._id, likeBy: userId})
          .select("_id")
          .lean())
      : false;
  }
...
const updateComment = asyncHandler(async (req, res) => {
  ...
  if (comment?.owner?.toString() !== req.user?._id?.toString()) {
    throw new ApiError(
      400,
      "Only the owner of this comment can edit the comment"
    );
  }
...
const deleteComment = asyncHandler(async (req, res) => {
  ...
  if (comment?.owner?.toString() !== req.user?._id?.toString()) {
    throw new ApiError(
      400,
      "Only the owner of this comment can delete the comment"
    );
  }
  ...
  if (!commentLikeDocuments) {
    throw new ApiError(
      400,
      "Failed to delete like documents corresponding to the comment"
    );
  }
